...
Stav: **LOKÁLNÍ ARTEFAKT PASS / EXTERNÍ DEMO GATE ČEKÁ** (2026-09-01)
...
- Entrypoint selhává při neznámém/konfliktním prostředí, HTTP/wildcard origins, chybějícím 32znakovém encryption key, R2 nebo heartbeat konfiguraci a nezapisovatelném Volume.
...
- Cleanup běží 03:30 UTC, storage kontrola každou hodinu. Backup, cleanup a healthy storage posílají oddělené heartbeat signály bez logování jejich URL.
...
- dokončit manuální Phase 6 gate;
- vytvořit izolované Fly, Vercel, R2 a Better Stack demo prostředky a dodat jejich credentials/domény;
- deploynout immutable image a frontend, vytvořit odděleného `_superusers` a normálního `admins` vlastníka;
- naplnit záměrná demo data přes migrace/admin, spustit celý demo acceptance matrix;
- vytvořit skutečnou R2 zálohu a obnovit ji do dočasné izolované Fly App/Volume včetně regenerace thumbu;
- zaznamenat image digest, migration head, monitor/backup stav a globální `noindex`.
